...
public class AddBulkChapterBackTask extends AsyncTask<String, Void, String> {
    Context mContext;
    public AddBulkChapterBackTask(Context mContext){
    ...
    protected String doInBackground(String... params) {
       ...
       /* String exmPaperId = params[2];
        String sectionId = params[3];
        String classId = params[4];*/
        //String schoolId = params[3];
        String json_url = Constants.addChaptersBulk + subjectId;
        try {
    ...
    protected void onPostExecute(String result) {
        if (result != null && !result.isEmpty() && result.equalsIgnoreCase("TRUE")){
            Toast.makeText(mContext, "Chapters Added...!", Toast.LENGTH_LONG).show();


        }
       /* else if( result.equalsIgnoreCase("FALSE"))
        {
            Toast.makeText(mContext, "chapters already Added...!", Toast.LENGTH_LONG).show();
